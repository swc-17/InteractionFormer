...
class GaussianNoise:
    ...
    def get_schedulable_attributes(self) -> List[ParameterToScale]:
        ...
        return [
            ParameterToScale(
                self.mean,
                param_name=f'{self.mean}'.split('=')[0].split('.')[1],
                scaling_direction=ScalingDirection.MAX,
            ),
            ParameterToScale(
                self.std,
                param_name=f'{self.std}'.split('=')[0].split('.')[1],
                scaling_direction=ScalingDirection.MAX,
            ),
        ]
...
class UniformNoise:
    ...
    def get_schedulable_attributes(self) -> List[ParameterToScale]:
        ...
        return [
            ParameterToScale(
                param=self.low,
                param_name=f'{self.low}'.split('=')[0].split('.')[1],
                scaling_direction=ScalingDirection.MAX,
            ),
            ParameterToScale(
                param=self.high,
                param_name=f'{self.high}'.split('=')[0].split('.')[1],
                scaling_direction=ScalingDirection.MAX,
            ),
        ]
