...
class KinematicHistoryAgentAugmentor(AbstractAugmentor):
    ...
    * minimize dist(perturbed_trajectory, ground_truth_trajectory)
    ...
    def safety_check(self, ego: npt.NDArray[np.float32]) -> bool:
        """
    ...
    def augment(self, ego, targets):
        ...
        ego_velocity_new = np.linalg.norm(np.diff(ego_perturb[:, :2], axis=0), axis=1) * 10
        x_real_curr = [ego_perturb[-1][0], ego_perturb[-1][1], ego_perturb[-1][2], ego_velocity_new[-1]]
        targets_traj = np.copy(targets)
        _optimizer = ConstrainedNonlinearSmoother(len(targets_traj), self._dt)
        targets_traj = np.concatenate([ego_perturb[[-1]], targets_traj], axis=0)
        _optimizer.set_reference_trajectory(x_real_curr, targets_traj)
        try:
            sol = _optimizer.solve()
        except RuntimeError:
            logger.error("Smoothing failed with status %s! Use G.T. instead" % sol.stats()['return_status'])
        ...
        if not sol.stats()['success']:
            logger.warning("Smoothing failed with status %s! Use G.T. instead" % sol.stats()['return_status'])
            return ego, targets
        future_ego_perturb: npt.NDArray[np.float32] = np.vstack(
            [
                sol.value(_optimizer.position_x),
                sol.value(_optimizer.position_y),
                sol.value(_optimizer.yaw),
        ...
        future_ego_perturb = future_ego_perturb.T[1:]

        if self.safety_check(ego_perturb) and self.safety_check(future_ego_perturb):
            return np.float32(ego_perturb), np.float32(future_ego_perturb)
        else:
            return ego, targets
    ...
    def augmentation_probability(self) -> ParameterToScale:
        ...
        return ParameterToScale(
            ...
            param_name=f'{self._augment_prob}'.split('=')[1].split('.')[1],
