...
class AttentionLayer(MessagePassing):
    ...
    def __init__(self,
        ...
        if has_query_pos_emb:
            self.attn_prenorm_query_r = nn.LayerNorm(hidden_dim)
    ...
    def forward(self,
                ...
                query_r: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        if self.has_query_pos_emb and query_r is not None:
            query_r = self.attn_prenorm_query_r(query_r)
        x = x + self.attn_postnorm(self._attn_block(x_src, x_dst, r, edge_index, query_r))
    ...
    def _attn_block(self,
        ...
        if self.has_query_pos_emb and query_r is not None:
            q = q + self.to_q_r(query_r).view(-1, self.num_heads, self.head_dim)
